/* Stage 11: weighted sampling without replacement over audible visible components.
...
function msg_int(n){if(inlet===5){n=Number(n);if(isFinite(n))componentCount=Math.max(2,Math.min(255,Math.round(n)));}}
...
function remove(){
    ...
    else {error("Stage 11: use remove <integer> or remove X partials <integer>\n");return;}
    n=Number(n);
    if(!isFinite(n)||n<0||Math.floor(n)!==n){error("Stage 11: removal count must be a nonnegative integer\n");return;}
    if(layers[1].length!==componentCount||layers[2].length!==componentCount){error("Stage 11: visible amplitude lists are not ready\n");return;}
