...
const main=read('_ec_sinusoids_synth_stage_v16.maxpat'),m=boxes(main);
const shapers=[read('Envelope-Shaper_01_v16.maxpat'),m['obj-21'].patcher,m['envelope-shaper01'].patcher];
const c={outlet:()=>{}};vm.createContext(c);vm.runInContext(fs.readFileSync(path.join(__dirname,'dynamic_shape_library_v16.js'),'utf8'),c);
