# v16 — score-message models

Open `_ec_sinusoids_synth_stage_v16.maxpat` with its sibling dependencies. Double-click `p Send_Message_Space_v16` on the main page. The same gallery is also supplied as `Send_Message_Space_v16.maxpat`.

The gallery now teaches how to build a separate score. It contains simple messaging recipes, 77 self-contained cue modules organized by category, a complete timed-score example, and your existing hands-on interface under `p Live_Controls_v16`.

## Copy a cue into a score

1. Open a category in Send_Message_Space. Use normal patching view so you can see connections.
2. Unlock the patch. Select the whole `p Cue_…_v16` object and copy it. You can also copy its adjacent example message and connecting cord.
...
4. Connect a timed source to its inlet. For example, a bang through `delay 1000` can trigger the message `110. 500.` into `p Cue_Source_Base_Hz_v16`.
5. Keep the v16 synthesis engine open. The cue's optional outlet reports the named command for logging; its internal forward already sends it to the engine.
...
The ramp duration belongs to the **cue inlet**, not to the engine's scalar receive object. For example, the cue accepts `110. 500.` but sends `110.` to `receive Source_Base_Hz_v16` as its final numeric payload. Its named monitor/outlet reports `Source_Base_Hz_v16 110.`. The named-message recipe strips the name before forwarding the payload.

## Timing and cancellation

`p Example_Timed_Score_v16` is a self-contained 4.5-second example, also supplied as a standalone patch. Its first inlet starts/restarts; its second inlet cancels. The visible START and STOP buttons do the same. Start cancels the previous run before scheduling fresh offsets. Stop sends `stop` to all delays and cue modules, cancelling pending events and active cue lines.
...
The receive names are shared among open v16 instances; this is not an instance-isolated instrument. Use one v16 engine for a score. No v14 receivers are addressed.
...
- `CONTROL_REFERENCE_v16.md`: every destination, type, bounds and example, plus shape and registration indices.
- `CHANGES_score_models_v16.md`: exact scope, preservation record, tests and Max checklist.
- `test_results_v16.json`: existing and new automated checks.
- `verify_score_models_v16.py`: portable preservation, dependency, graph and layout checks.

The source is the latest supplied `ec_sinusoids_synth_v16.zip`. Original synthesis dependencies and live views are preserved. Keep CNMAT `sinusoids~` installed as in your existing setup.
