...
const r=load('weighted_partial_removal_v16.js');configure(r,24,Array.from({length:24},(_,i)=>24-i));r.remove('X','partials',10);
...
const patch=JSON.parse(fs.readFileSync(path.join(__dirname,'_ec_sinusoids_synth_stage_v16.maxpat'))).patcher;
...
const gen=load('cnmat_sinusoid_model_generator_v16.js');
engine=load('multislider_interpolation_engine_v16.js',(channel,message,n)=>{
...
remover=load('weighted_partial_removal_v16.js',(out,command)=>{if(out===0){engine.inlet=10;engine[command[0]](...command.slice(1));}});
const library=load('dynamic_shape_library_weights_v16.js',(out,values)=>{if(out===0){remover.inlet=1;remover.list(...values);}});
...
const lib=load('dynamic_shape_library_weights_v16.js');lib.inlet=0;lib.msg_int(48);lib.inlet=1;lib.msg_int(names.indexOf('Triple Gaussian'));
...
function graph(p){const map=new Map(p.boxes.map(x=>[x.box.id,x.box]));assert.equal(map.size,p.boxes.length);for(const l of p.lines||[]){const q=l.patchline;assert(map.has(q.source[0]));assert(map.has(q.destination[0]));assert(q.source[1]<map.get(q.source[0]).numoutlets);assert(q.destination[1]<map.get(q.destination[0]).numinlets);}for(const x of p.boxes){if(x.box.patcher)graph(x.box.patcher);if(x.box.text?.startsWith('js '))assert(x.box.text.endsWith('_v16.js'));}}
graph(patch);
