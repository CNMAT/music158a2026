...
const p=JSON.parse(fs.readFileSync(path.join(__dirname,'_ec_sinusoids_synth_stage_v16.maxpat'))).patcher;assert(p.lines.some(x=>x.patchline.source[0]==='copy-amplitudes14'&&x.patchline.destination[0]==='syn-interpolation-js'&&x.patchline.destination[1]===10));
