...
const g=load('cnmat_sinusoid_model_generator_v16.js',(...a)=>{if(a[0]===0)models++;});let models=0,e;
e=load('multislider_interpolation_engine_v16.js',(ch,msg,n)=>{if(ch===3){g.inlet=7;g[msg]();}else if(msg!=='size'){g.inlet=ch;g.list(...msg);}});
...
const p=JSON.parse(fs.readFileSync(path.join(__dirname,'_ec_sinusoids_synth_stage_v16.maxpat'))).patcher;
...
for(const name of ['dynamic_shape_library_v16.js','dynamic_shape_library_weights_v16.js','random_multislider_values_v16.js']){const c=load(name);c.inlet=0;c.msg_int(256);assert.equal(c.componentCount,256);}
