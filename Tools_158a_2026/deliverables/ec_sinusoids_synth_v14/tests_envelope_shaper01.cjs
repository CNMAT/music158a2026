...
assert.equal(e['obj-shape-count-48'].text,'45');assert.equal(e['obj-round-output'].text,'t l');
...
console.log('PASS: recursive graph/port validation; all 24 exact stage-14 templates, including zero; normalized ranges; precise list path; embedded routing without duplicate broadcast; standalone/embedded equivalence; no top-level object rectangle overlaps.');
