...
const patch=JSON.parse(fs.readFileSync(path.join(__dirname,'_ec_sinusoids_synth_stage_14.maxpat'))).patcher;let safe=[],targetDisplay;
const transform=load('sinusoid_model_transform_14.js',(out,...v)=>{if(out===0)safe=Array.from(v[0]);});
const gen=load('cnmat_sinusoid_model_generator_14.js',(out,...v)=>{if(out===4){transform.inlet=0;transform.list(...v[0]);}});
...
const controller=load('global_retuning_controller_14.js',(out,...v)=>{if(out===0)generatorBase(v[0]);if(out===1)transform.tuning(...v.slice(1));if(out===3)targetDisplay=v[1];});
