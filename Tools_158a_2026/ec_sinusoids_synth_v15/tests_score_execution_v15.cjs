...
const gallery=read('Send_Message_Space_v15.maxpat');const models={};
...
class World{
 ...
 send(name,msg){const a=atoms(msg);this.sent.push({at:this.time,name,msg:structuredClone(msg)});
  const c=['Partial_Drift_v15','Partial_Amplitude_v15','Partial_Anchor_v15'].indexOf(name);
  if(c>=0){if(a[0]==='get'){this.bus(name.slice(0,-4)+'_current_v15',this.partial[c][a[1]-1]);return;}this.partial[c][a[0]-1]=a[1];}
  if(a.length===1&&typeof a[0]==='number')this.bus(name.slice(0,-4)+'_state_v15',a[0]);
  this.bus(name,msg);
 }
}
class Graph{
...
let w=new World(),g=new Graph(models.Source_Base_Hz_v15,w);assert.equal(w.sent.length,0);w.bus('Source_Base_Hz_state_v15',60);g.input([100,40]);w.advance(20);assert.equal(w.sent.at(-1).msg[0],80);w.advance(40);assert.equal(w.sent.at(-1).msg[0],100);
...
w=new World();w.partial[1][2]=.2;w.partial[1][3]=.4;let a=new Graph(models.Partial_Amplitude_v15,w),b=new Graph(models.Partial_Amplitude_v15,w);
...
w=new World();g=new Graph(models.Amplitude_Shape_v15,w);g.input(7);assert.equal(w.sent.length,1);assert.equal(w.sent[0].msg[0],7);g.input(['stop']);assert.equal(w.sent.length,1);
g=new Graph(models.All_Off_v15,w);g.input(['stop']);assert.equal(w.sent.length,1);g.input('bang');assert.equal(w.sent.length,2);
// Timed example opens silently, cancels future events and ramp work, and can restart without stale cues.
w=new World();g=new Graph(read('Example_Timed_Score_v15.maxpat'),w);assert.equal(w.sent.length,0);g.input('bang');w.advance(300);assert(w.sent.some(x=>x.name==='Envelope_Trigger_v15'&&x.at===300));g.input('bang',1);const stopped=w.sent.length;w.advance(5000);assert.equal(w.sent.length,stopped);
g.input('bang');w.advance(5300);assert(w.sent.some(x=>x.name==='Envelope_Trigger_v15'&&x.at===5300));w.advance(9500);assert(w.sent.some(x=>x.name==='All_Off_v15'&&x.at===9500));
