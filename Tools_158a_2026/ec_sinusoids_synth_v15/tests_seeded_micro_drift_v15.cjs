...
function make(){let raw=[],live=0;const c={Math,Date:Clock,inlet:0,isFinite,error:()=>{},arrayfromargs:a=>Array.from(a),Task:function(){this.cancel=()=>{};this.repeat=()=>{};},outlet:(o,v,n)=>{if(o===4)raw=Array.from(v);if(o===5)live=n;}};vm.createContext(c);vm.runInContext(fs.readFileSync(path.join(__dirname,'cnmat_sinusoid_model_generator_v15.js'),'utf8'),c);c.raw=()=>raw;c.live=()=>live;return c;}
...
vm.createContext(tc);vm.runInContext(fs.readFileSync(path.join(__dirname,'sinusoid_model_transform_v15.js'),'utf8'),tc);
...
for(const [a,out,b,bi] of [['obj-32',0,'syn-js',8],['syn-js',4,'rt14-transform',0],['rt14-transform',0,'obj-5',0],['obj-5',0,'obj-27',0],['obj-29',0,'obj-signal-in',0],['syn-js',5,'obj-32',0]])assert(patch.lines.some(x=>{const l=x.patchline;return l.source[0]===a&&l.source[1]===out&&l.destination[0]===b&&l.destination[1]===bi;}));
...
assert.equal(patch.boxes.find(x=>x.box.id==='obj-27').box.text,'send model-s_v15');assert.equal(patch.boxes.find(x=>x.box.id==='obj-29').box.text,'r model-s_v15');
