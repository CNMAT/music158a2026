...
const g=load('cnmat_sinusoid_model_generator_v15.js',(...a)=>{if(a[0]===0)models++;});let models=0,e;
e=load('multislider_interpolation_engine_v15.js',(ch,msg,n)=>{if(ch===3){g.inlet=7;g[msg]();}else if(msg!=='size'){g.inlet=ch;g.list(...msg);}});
...
for(const name of ['dynamic_shape_library_v15.js','dynamic_shape_library_weights_v15.js','random_multislider_values_v15.js']){const c=load(name);c.inlet=0;c.msg_int(256);assert.equal(c.componentCount,256);}
