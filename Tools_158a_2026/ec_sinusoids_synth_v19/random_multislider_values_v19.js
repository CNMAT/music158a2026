...
function randomList(signed) {
    var result = [];
    var i;
    for (i = 0; i < componentCount; i++) {
        result[i] = Math.round((signed ? Math.random()*2-1 : Math.random()) * 1000000.0) / 1000000.0;
    }
    return result;
}
...
function msg_int(value) {
    if (inlet === 0) {
        componentCount = clipInteger(value, 2, 256);
    }
}
