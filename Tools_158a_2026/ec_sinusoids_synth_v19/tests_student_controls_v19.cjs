...
const main=read('_ec_sinusoids_synth_stage_v19.maxpat'),stops=read('ec_sinusoid_synth_stops_v19.maxpat'),stand=read('Envelope-Shaper_01_v19.maxpat');
const registry=read('controls_v19.json'),boxes=p=>Object.fromEntries(p.boxes.map(x=>[x.box.id,x.box]));
const gallery=boxes(main.patcher)['send-message-space-v19'].patcher;
...
function graph(p){const b=boxes(p);assert.equal(Object.keys(b).length,p.boxes.length);for(const {patchline:l} of p.lines||[]){assert(b[l.source[0]]&&b[l.destination[0]],JSON.stringify(l));assert(l.source[1]>=0&&l.source[1]<b[l.source[0]].numoutlets,JSON.stringify(l));assert(l.destination[1]>=0&&l.destination[1]<b[l.destination[0]].numinlets,JSON.stringify(l));}
 for(const box of Object.values(b)){if(box.patcher)graph(box.patcher);if(box.text?.startsWith('js ')){const file=box.text.slice(3).split(' ')[0];assert(file.endsWith('_v19.js'),file);assert(fs.existsSync(path.join(__dirname,file)),file);}if(box.filename?.endsWith('.js'))assert(fs.existsSync(path.join(__dirname,box.filename)));}}
for(const file of fs.readdirSync(__dirname).filter(f=>f.endsWith('.maxpat')))graph(read(file).patcher);
assert.deepStrictEqual(boxes(read('Send_Message_Space_v19.maxpat').patcher)['live-controls'].patcher,space);
// Every migrated original node, position, attribute, and wire is preserved.
for(const old of read('baseline_structure_v19.json')){const p=at(read(old.file),old.path);if(old.box){const b=JSON.parse(JSON.stringify(boxes(p)[old.id]));delete b.patcher;assert.deepStrictEqual(b,old.box,old.file+'/'+old.path+'/'+old.id);}else for(const l of old.lines)assert(p.lines.some(x=>JSON.stringify(x)===JSON.stringify(l)),JSON.stringify(l));}
...
const names=new Set();for(const e of registry){assert(!names.has(e.name));names.add(e.name);assert(e.name.endsWith('_v19'));
...
c.partialget(1,200);assert.deepStrictEqual(feedback.at(-1),['Partial_Amplitude_current_v19',.73]);
...
console.log('PASS: '+registry.length+' named receivers; complete sender chains; 20ms/5ms ramps; discrete integer control; preserved revised baseline nodes/wires; no presentation overlaps; silent sender initialization; v19 dependencies; indexed edits, hidden memory, input validation, current-value queries, interrupted morphs and octave-family dispatch.');
