...
const r=load('weighted_partial_removal_v19.js');configure(r,24,Array.from({length:24},(_,i)=>24-i));r.remove('X','partials',10);
...
const patch=JSON.parse(fs.readFileSync(path.join(__dirname,'_ec_sinusoids_synth_stage_v19.maxpat'))).patcher;
const ids=new Map(patch.boxes.map(x=>[x.box.id,x.box]));const names=ids.get('rm14-shape-menu').items.filter(x=>x!==',');
let remover,engine;const display=[[],[],[]];
const gen=load('cnmat_sinusoid_model_generator_v19.js');
engine=load('multislider_interpolation_engine_v19.js',(channel,message,n)=>{
...
remover=load('weighted_partial_removal_v19.js',(out,command)=>{if(out===0){engine.inlet=10;engine[command[0]](...command.slice(1));}});
const library=load('dynamic_shape_library_weights_v19.js',(out,values)=>{if(out===0){remover.inlet=1;remover.list(...values);}});
...
const lib=load('dynamic_shape_library_weights_v19.js');lib.inlet=0;lib.msg_int(48);lib.inlet=1;lib.msg_int(names.indexOf('Triple Gaussian'));
...
function graph(p){const map=new Map(p.boxes.map(x=>[x.box.id,x.box]));assert.equal(map.size,p.boxes.length);for(const l of p.lines||[]){const q=l.patchline;assert(map.has(q.source[0]));assert(map.has(q.destination[0]));assert(q.source[1]<map.get(q.source[0]).numoutlets);assert(q.destination[1]<map.get(q.destination[0]).numinlets);}for(const x of p.boxes){if(x.box.patcher)graph(x.box.patcher);if(x.box.text?.startsWith('js '))assert(x.box.text.endsWith('_v19.js'));}}
graph(patch);
...
if(fs.existsSync(oldPath)){for(const {box} of JSON.parse(fs.readFileSync(oldPath)).patcher.boxes){const b=ids.get(box.id.replace(/rt10/g,'rt13').replace(/stage10/g,'stage12'));assert.deepEqual(b.patching_rect,box.patching_rect);for(const k of Object.keys(box).filter(k=>/color/.test(k)))assert.deepEqual(b[k],box[k]);}}
