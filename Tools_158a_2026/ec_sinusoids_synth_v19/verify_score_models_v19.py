...
main=read('_ec_sinusoids_synth_stage_v19.maxpat');g=boxes(main['patcher'])['send-message-space-v19']['patcher'];stand=read('Send_Message_Space_v19.maxpat')['patcher'];pres=read('score_preservation_v19.json')
trim=copy.deepcopy(main);trim_boxes=boxes(trim['patcher']);trim_boxes['send-message-space-v19'].pop('patcher')
if 'send-message-space-note-v19' in trim_boxes:trim_boxes['send-message-space-note-v19'].pop('text',None)
...
manifest=read('score_models_manifest_v19.json');expected={e['name'] for e in manifest['controls']}
models={}
...
rolled_back_cues={'Audio_On_v19','Output_Gain_dB_v19','Library_Breakpoints_v19','Library_Duration_ms_v19','Library_Shape_v19','Library_Store_Index_v19','Library_Store_v19','Library_Emit_v19','Library_Points_v19'}
assert expected-set(models)==rolled_back_cues
assert set(models)-expected=={'ADSR_Recall_v19'}
...
score=read('Example_Timed_Score_v19.maxpat')['patcher'];b=boxes(score)
...
for name,digest in read('score_baseline_hashes_v19.json').items():
 if Path(name).suffix in ['.js'] or name in ['Envelope-Shaper_01_v19.maxpat','ec_sinusoid_synth_stops_v19.maxpat','ec_sinusoid_synth_stops_research_v19.maxpat','controls_v19.json']:
