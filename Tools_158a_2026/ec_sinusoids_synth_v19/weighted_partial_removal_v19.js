/* Stage 15: weighted sampling without replacement over audible visible components.
...
function remove(){
    ...
    else {error("Stage 15: use remove <integer> or remove X partials <integer>\n");return;}
    n=Number(n);
    if(!isFinite(n)||n<0||Math.floor(n)!==n){error("Stage 15: removal count must be a nonnegative integer\n");return;}
    if(layers[1].length!==componentCount||layers[2].length!==componentCount){error("Stage 15: visible amplitude lists are not ready\n");return;}
