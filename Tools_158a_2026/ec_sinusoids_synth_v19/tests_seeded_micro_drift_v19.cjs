...
function make(){let raw=[],live=0;const c={Math,Date:Clock,inlet:0,isFinite,error:()=>{},arrayfromargs:a=>Array.from(a),Task:function(){this.cancel=()=>{};this.repeat=()=>{};},outlet:(o,v,n)=>{if(o===4)raw=Array.from(v);if(o===5)live=n;}};vm.createContext(c);vm.runInContext(fs.readFileSync(path.join(__dirname,'cnmat_sinusoid_model_generator_v19.js'),'utf8'),c);c.raw=()=>raw;c.live=()=>live;return c;}
...
vm.createContext(tc);vm.runInContext(fs.readFileSync(path.join(__dirname,'sinusoid_model_transform_v19.js'),'utf8'),tc);
...
assert.equal(patch.boxes.find(x=>x.box.id==='obj-27').box.text,'send model-s_v19');assert.equal(patch.boxes.find(x=>x.box.id==='obj-29').box.text,'r model-s_v19');
